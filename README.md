...
Например у нас есть смарт-процесс "Отпуск" `\Chelbit\Smart\CRM\Vacation` и этого смарт-процесса в настройках связи включили привязку к сделке. Таким образом мы имеев в карточке смарт-процесса "Отпуск" поле сделка. Это означает, что сделка является родителем отпуска.
```php
...
```
> ⚠️ **Внимание!**  
> Методы получения родителей и детей проверяют настрйоки связей и выбросят исключение если запрашиваемый тип связи не возможен!
...
|`getInstanceById()`||`int $id` - ID элемента CRM для которго нужно создать объект|`self`|
|`getItems()`|Получение списка элементоа|`array $params` - аналогично штатному методу `$factory->getItems()`|`self[]`|
|`getItemIdentifier()`|||\Bitrix\Crm\ItemIdentifier|
|`getParentItem()`|Получение родительского элемента|`string $class` - имя класса объекта родителя|`\ChelBit\Smart\Core\Base\Smart or null` - объект класса переаднного в параметр или null  если родитель не указан|
