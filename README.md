...
```
Для примеры выше можно сказать, что отпуск является дочерним элементом для сделки. И предположим, что к одной сделке привязано много элеентов отпусков. Мы можем их все получить следующим образом.
```php
